...
//   }
  ...
  async checkToken(token) {
    ...
    try {
      ...
      const fail = error.name === 'TokenExpiredError' ? 'token 已过期! 请重新获取令牌' : 'Token 令牌不合法!222';
    ...
    if (!t || t !== token) {
      socket.emit(id, ctx.helper.parseMsg('error', 'Token 令牌不合法!3333'));
      return false;
    }
